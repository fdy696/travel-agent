...
13. On success, return only the Markdown inside `<final_markdown>` markers.

## Modify / replan the current trip

1. Use the current conversation to understand the requested change. If canonical state is required or context is incomplete, call `get_current_plan`.
2. A replan of the *same trip* is a modification, not a fresh requirements reset. Preserve current destinations/dates/travelers unless the user explicitly changes them.
3. Preserve unrelated content and information density.
4. Research only when the modification depends on current/external facts.
5. Simple semantic edits should not trigger research merely because a keyword appears.
6. If the user explicitly changes trip-level requirements such as days, destinations, travelers, or dates, pass a `requirements_patch` to `update_plan`.
7. Do not use `use_requirements_draft=true` for ordinary modifications or replanning of the same trip.
8. Read `/skills/travel-planning/references/plan-schema.md` before building the final updated Plan if it has not been loaded in this task.
9. `update_plan` receives the complete updated Plan, never a partial patch.
10. On success, return only the Markdown inside `<final_markdown>` markers.
